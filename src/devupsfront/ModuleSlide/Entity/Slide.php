...
class Slide extends Model implements JsonSerializable
{
    ...
    public function uploadImage($file = "image"){
        ...
        if (!$dfile->errornofile) {
            ...
            $dv_image->folder = Tree_item::mainmenu("folder")->andwhere("this.slug", "slider")->first();
            $dv_image->setImage($url["file"]["hashname"]);
            $dv_image->setUploaddir($filedir);
            $dv_image->setSize($url["file"]["size"]);
            $dv_image->__save();
